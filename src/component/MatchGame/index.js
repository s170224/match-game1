...
import MatchGameItems from '../MatchGameItems'
...
class MatchGame extends Component {
  ...
  shuffledImage = () => {
    const {category0, newId} = this.state
    // const filterThumbnail = imagesList.sort(() => Math.random() - 0.5)
    const val = imagesList[Math.floor(Math.random() * imagesList.length)]
    // this.setState({newId: val.id})

    return val.thumbnailUrl
  }
  ...
  getFilterMatchGameList = () => {
    const {category} = this.state
    const filterMatchGames = imagesList.filter(
      eachImage => eachImage.category === category,
    )
    return filterMatchGames
  }

  updateScoreMatchItem = id => {
    const {score, newId} = this.state
  ...
  render() {
    const {category, score, newId, sec} = this.state
    console.log(newId)
    const filterMatchGameList = this.getFilterMatchGameList()
    const shuffledListImage = this.shuffledImage()

    // const updateScoreMatchItem1 = this.updateScoreMatchItem()
    // console.log(updateScoreMatchItem1)
      ...
      <div className="main-container">
        <div className="navContainerS">
          ...
          <div className="count-container">
            ...
            <p className="countScore">{score}</p>
          </div>
          <div className="sec-con">
            <img
              ...
              className="timeImage"
            />
            <p className="secpara">{sec} sec</p>
          </div>
        </div>
        <div className="thumb-con">
          <button type="button" onClick={this.shuffledImage}>
            <img src={shuffledListImage} className=" thubnailImage" />
          </button>
        </div>
        <ul className="fruit-con">
        ...
        <ul className="mathListContainer">
          {/* {filterMatchGameList.map(e => (
            <li key={e.id}>
              <button onClick={() => this.updateScoreMatchItem(e.id)}>
                <img src={e.imageUrl} />
              </button>
            </li>
          ))} */}
          {filterMatchGameList.map(eachMatch => (
            <MatchGameItems
              key={eachMatch.id}
              eachItemMatchGame={eachMatch}
              updateScoreMatchItem={this.updateScoreMatchItem}
            />
          ))}
        </ul>
      </div>
